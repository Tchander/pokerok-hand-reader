...
export function useHandHandler() {
  ...
  async function handHandler(hand: string[]) {
    ...
    let isPreFlopThreeBetSituation: boolean = false;
    ...
    for (const playerAction of pokerHand.actions.preFlopActions) {
      ...
      if (isHero(id) && preFlopRaiseCounter === 1) {
        isPreFlopThreeBetSituation = true;
      }
      ...
      if (action === PlayerAction.RAISE) {
        ...
        if (isHero(id) && preFlopRaiseCounter === 0 && preFlopCallCounter >= 1) {
          counters.value.preFlopSqueeze++;
        }

        if (isHero(id) && isHeroPreFlopRaiser && preFlopRaiseCounter === 1) {
          counters.value.preFlopThreeBets++;
        }

        if (!isHero(id) && isHeroPreFlopRaiser && preFlopRaiseCounter >= 1) {
          counters.value.numberOfFoldThreeBetsSituations++;
        }
      ...
      if (action === PlayerAction.FOLD) {
        if (isHero(id) && isHeroPreFlopRaiser && preFlopRaiseCounter > 1) {
          counters.value.foldPreFlopThreeBets++;
        }
      }
      ...
      if (action === PlayerAction.SHOW) {
        ...
        if (isHero(id)) {
          ...
          counters.value.sawFlopTimes++;
          counters.value.sawTurnTimes++;
          counters.value.sawRiverTimes++;
          counters.value.sawShowDownTimes++;
        }
      }
    }
    ...
    if (isPreFlopSqueezeSituation) {
      counters.value.numberOfSqueezeSituations++;
    }

    // Was PreFlop 3-bet situation
    if (isPreFlopThreeBetSituation) {
      counters.value.numberOfThreeBetSituations++;
    }

    // Is Hero Put Chips Into Pot
    if (isHeroPutIntoPot) {
      counters.value.putIntoPot++;
    }
    ...
    if (wonShowDownTimes / pokerHand.boardsAmount > 0.5) {
      counters.value.wonShowDownTimes++;
    }

    counters.value.numberOfHands++;
  }
